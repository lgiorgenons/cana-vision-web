"""Renderiza um mapa interativo com composição RGB (true color) a partir de bandas Sentinel-2.

As bandas B04 (vermelho), B03 (verde) e B02 (azul) devem estar disponiveis como GeoTIFFs,
como gerado pelo pipeline em `data/processed/<produto>/`. O resultado e um HTML com visualização
...
def _stretch(array: np.ndarray, lower: float = 2, upper: float = 98) -> np.ndarray:
    ...
    if not np.any(finite):
        raise RuntimeError("Banda sem valores válidos.")
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--output", type=Path, default=Path("mapas/truecolor.html"), help="Arquivo HTML de saída.")
